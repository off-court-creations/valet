...
const BOX_SIZE = '0.75em';

const Branch = styled('ul')<{ $line: string; $root?: boolean }>`
  list-style: none;
  margin: 0;
  padding-left: ${({ $root }) => ($root ? 0 : `calc(1rem - ${BOX_SIZE} / 2)`) };
  position: relative;
  ${({ $root, $line }) =>
    !$root &&
    `&::before{content:'';position:absolute;top:0;bottom:0;left:calc(${BOX_SIZE}/-2);border-left:1px solid ${$line};}`}
`;
...
const BranchItem = styled('li')<{ $line: string }>`
  ...
  &::before {
    ...
    position: absolute;
    top: 0.875rem;
    left: calc(${BOX_SIZE} / -2);
    width: calc(1rem - ${BOX_SIZE} / 2);
