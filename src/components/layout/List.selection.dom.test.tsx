...
/* ─────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────── */
describe('List deprecated selection aliases (jsdom)', () => {
  it('`selectable` still enables single selection and warns once across re-renders', () => {
    const onSelectionChange = vi.fn();
    const ui = () => (
      <List<Item>
        data={data}
    ...
    const { container, rerender } = renderStrict(ui());
    expect(container.querySelector('[role="listbox"]')).not.toBeNull();
    act(() => rows(container)[0].dispatchEvent(new MouseEvent('click', { bubbles: true })));
    expect(onSelectionChange).toHaveBeenCalledWith(data[0], 0);

    rerender(ui()); /* must not re-warn */
  ...
  it('`getKey` still keys rows and warns once', () => {
    const { container } = renderStrict(
      <List<Item>
        data={data}
        getTitle={(i) => i.label}
    ...
    const warns = deprecationWarns();
    expect(warns).toHaveLength(1);
    expect(warns[0]).toContain('`getKey`');
...
/* ─────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────── */
describe('List canonical + deprecated together (jsdom)', () => {
  it('`selectionMode="none"` wins over `selectable`, and `selectable` still warns once', () => {
    const onSelectionChange = vi.fn();
    const { container } = renderStrict(
      <List<Item>
        data={data}
        getTitle={(i) => i.label}
        selectionMode='none'
        selectable /* stale: canonical disables selection */
        onSelectionChange={onSelectionChange}
      />,
    );
    expect(container.querySelector('[role="listbox"]')).toBeNull();
  ...
  it('`getItemKey` wins over `getKey`; `getKey` still warns once', () => {
    const onSelectionChange = vi.fn();
    const { container } = renderStrict(
      <List<Item>
        data={data}
    ...
    /* behavior is identity-driven, so the win is observable only via the warn;
       selection still works under the canonical key */
    act(() => rows(container)[1].dispatchEvent(new MouseEvent('click', { bubbles: true })));
    expect(onSelectionChange).toHaveBeenCalledWith(data[1], 1);
    const warns = deprecationWarns();
    expect(warns).toHaveLength(1);
