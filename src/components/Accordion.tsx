...
import React, {
  ...
  export interface AccordionProps
    extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onChange'>,
      Presettable {
    /** Default open indices when uncontrolled. */
    defaultOpen?: number | number[];
    /** Controlled open indices. */
    open?: number | number[];
    /** When false (default) acts like a disclosure list – only one item open. */
    multiple?: boolean;
    /** Callback every time open indices change. */
    onOpenChange?: (open: number[]) => void;
    /** `h` tag used for headers (h3 default). */
  ...
  export interface AccordionItemProps
    extends React.HTMLAttributes<HTMLDivElement>,
      Presettable {
    /** Title for header button. */
    header: ReactNode;
    /** Unique index is injected automatically by parent. */
    index?: number;
    /** Disable user interaction for this item. */
    disabled?: boolean;
    /** Children content. */
    children: ReactNode;
  }
    ...
    return (
      ...
      >
        <HeaderTag style={{ margin: 0 }}>
          ...
          >
            {header}
            <Chevron aria-hidden $open={isOpen}>
              ▶
            </Chevron>
          </HeaderBtn>
        </HeaderTag>
