...
import React, {
  ...
  const Root = styled('div')<{
    ...
    `
      ...
      grid-template-columns: ${$placement === 'left' ? 'max-content 1fr'
                                                       : '1fr max-content'};
  ...
  const TabList = styled('div')<{
    ...
    gap: 0; /* no extra gaps; theme spacers can style outer wrapper */
  ...
  const TabBtn = styled('button')<{
    ...
    /* Don’t stretch when vertical (list width auto) */
    ${({ $orient }) =>
  ...
  const Panel = styled('div')`
    padding: 1rem 0;
    overflow: hidden; /* defensive – long content won’t push grid */
  `;
  ...
  export interface TabProps
    extends React.ButtonHTMLAttributes<HTMLButtonElement>,
      Presettable { index?: number; label?: ReactNode; }
  export interface TabPanelProps
    extends React.HTMLAttributes<HTMLDivElement>,
      Presettable { index?: number; keepMounted?: boolean; }
  ...
  export const Tabs: React.FC<TabsProps> & {
    ...
    /* split children into tabs / panels in order */
    const tabs: ReactElement[]   = [];
    const panels: ReactElement[] = [];
    React.Children.forEach(children, (child) => {
      if (!React.isValidElement(child)) return;
      const name = (child.type as any).displayName;
      if (name === 'Tabs.Tab')   tabs.push(
        React.cloneElement(child,   { index: tabs.length } as any));
      if (name === 'Tabs.Panel') panels.push(
        React.cloneElement(child, { index: panels.length } as any));
    });
    ...
    /* convenience for class merge */
    const cls = [p ? preset(p) : '', className].filter(Boolean).join(' ');
  
    /* order render to match grid tracks */
    const stripFirst =
    ...
    return (
      <TabsCtx.Provider value={ctx}>
        ...
        >
          {stripFirst && (
            <TabList $orientation={orientation}>{tabs}</TabList>
          )}
  
          {/* Panel ALWAYS occupies the opposite grid track */}
          <Panel>{panels}</Panel>
