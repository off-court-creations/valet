...
import React, {
  ...
  export interface CheckboxProps
    ...
    /** Identifier for FormControl binding (required when in form). */
    name: string;
    /** Optional label rendered next to the checkbox. */
    label?: ReactNode;
    /** Visual size token. */
    size?: CheckboxSize;
    /** Change callback emitting the new checked state. */
  ...
  const Box = styled('span')<BoxProps>`
    ...
    border: 2px solid ${({ $text, $checked, $primary }) =>
