...
const scenarios = [
  ...
  {
    id: 'hybrid:mcp-custom',
    template: 'hybrid',
    flags: ['--no-router', '--no-zustand', '--minimal', '--path-alias', 'app'],
    mcp: true,
    checks: ['agents:hybrid-custom'],
  },
  // R3F/three opt-in scenarios
  {
    ...
    flags: ['--three'],
    mcp: false,
    checks: ['three:deps', 'three:quickstart'],
  },
  {
    id: 'js:three',
    template: 'js',
  ...
  },
  {
    ...
    flags: ['--three'],
    mcp: true,
    checks: ['three:deps', 'three:quickstart', 'agents:r3f-policy'],
  },
...
async function runScenario(baseDir, s) {
  ...
  const typeOk =
    /TYPECHECK_STATUS:ok/.test(typeRes.out) || s.template === 'js' || typeRes.code === 0;
...
async function postChecks(appDir, s) {
  ...
  if (s.checks?.includes('no-router')) {
    ...
    if (pkg.dependencies && pkg.dependencies['react-router-dom'])
      fail('react-router-dom should be removed');
    if (exists(path.join(appDir, 'src', 'pages', 'second')))
      fail('second page directory should be removed');
  }

  // Minimal: second page removed
  if (s.checks?.includes('minimal')) {
    if (exists(path.join(appDir, 'src', 'pages', 'second')))
      fail('second page directory should be removed in minimal');
  }
  ...
  if (s.checks?.includes('alias:app')) {
    const vite =
      readFileSafe(path.join(appDir, 'vite.config.ts')) ||
      readFileSafe(path.join(appDir, 'vite.config.js')) ||
      '';
  ...
  if (s.checks?.includes('agents:ts-default')) {
    ...
    if (!/This is a TypeScript template\./.test(md))
      fail('AGENTS.md should mention TypeScript template');
    if (!/Typecheck:\s*`npm run -s typecheck`/.test(md))
      fail('AGENTS.md should include typecheck command');
    ...
    if (!/This is a TypeScript template\./.test(cd))
      fail('CLAUDE.md should mention TypeScript template');
    if (!/Typecheck:\s*`npm run -s typecheck`/.test(cd))
      fail('CLAUDE.md should include typecheck command');
  ...
  if (s.checks?.includes('agents:js-default')) {
    const md = readFileSafe(path.join(appDir, 'AGENTS.md')) || '';
    if (!md) fail('AGENTS.md should exist when --mcp');
    if (!/JavaScript-only template/.test(md))
      fail('AGENTS.md should mention JavaScript-only template');
    if (/Typecheck:\s*`npm run -s typecheck`/.test(md))
      fail('AGENTS.md should not include typecheck command for JS');
    if (!/Typecheck:\s*n\/a for JS template\./.test(md))
      fail('AGENTS.md should mark typecheck n/a for JS');

    const cd = readFileSafe(path.join(appDir, 'CLAUDE.md')) || '';
    if (!cd) fail('CLAUDE.md should exist when --mcp');
    if (!/JavaScript-only template/.test(cd))
      fail('CLAUDE.md should mention JavaScript-only template');
    if (/Typecheck:\s*`npm run -s typecheck`/.test(cd))
      fail('CLAUDE.md should not include typecheck command for JS');
    if (!/Typecheck:\s*n\/a for JS template\./.test(cd))
      fail('CLAUDE.md should mark typecheck n/a for JS');
  }
  ...
  if (s.checks?.includes('three:quickstart')) {
    ...
    if (!/from\s+"@react-three\/fiber"/.test(src))
      fail('Quickstart should import from @react-three/fiber');
  ...
  if (s.checks?.includes('agents:r3f-policy')) {
    const md = readFileSafe(path.join(appDir, 'AGENTS.md')) || '';
    if (!/R3F Pages Policy/.test(md))
      fail('AGENTS.md should include R3F Pages Policy when 3D is enabled on hybrid');
    if (!/implement them as `\.jsx` files/.test(md))
      fail('AGENTS.md should advise .jsx for R3F-heavy pages');
  }

  return { ok: msgs.length === 0, log: msgs.join('\n') };
}

// Fast packaging preflight (TEST-CI S9): run the no-dangling-symlink / bin /
// pack assertions before the expensive scaffold scenarios so a packaging
// regression fails in seconds, not minutes. Skippable with --no-pack-check
// (the weekly workflow keeps it on).
function packPreflight() {
  if (args.includes('--no-pack-check')) {
    logLine('[validate] pack preflight skipped (--no-pack-check)');
    return true;
  }
  logLine('[validate] pack preflight -> scripts/pack.test.mjs');
  const res = spawnSync(process.execPath, [path.join(ROOT, 'scripts', 'pack.test.mjs')], {
    cwd: ROOT,
    stdio: 'inherit',
  });
  return res.status === 0;
...
(async function main() {
  if (!packPreflight()) {
    logLine('[validate] FAILED: pack preflight');
    process.exit(1);
  }
