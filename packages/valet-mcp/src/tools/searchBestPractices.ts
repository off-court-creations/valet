...
export function registerSearchBestPractices(server: McpServer): void {
  server.registerTool(
    'valet__search_best_practices',
    {
      ...
      description:
        'Search best-practice entries across components and surface the most relevant advice.',
    ...
    async ({ query, limit, category, status }: ParamsType) => {
      ...
      const tokens = Array.from(
        new Set((normalizedQuery.match(/[a-z0-9]+/g) ?? []).filter((t) => t.length >= 2)),
      );
      ...
      for (const item of candidates) {
        ...
        if (wantStatus) {
          ...
          const matches =
            (idxStatus && statusFilter.has(idxStatus)) ||
            (docStatus && statusFilter.has(docStatus));
          if (!matches) continue;
        }
      ...
      results.sort(
        (a, b) => b.matchScore - a.matchScore || a.name.localeCompare(b.name) || a.index - b.index,
      );
