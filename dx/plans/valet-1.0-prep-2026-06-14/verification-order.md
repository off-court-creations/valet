...
- [x] **AppBar** _(agent-verified 2026-06-18 — modern already (intent vars, SSR-safe portal); added a coarse-pointer ≥44px floor for icon-only `navigation` buttons via `--valet-appbar-navbtn`; awaiting Ben's visual pass)_ · [x] **Drawer** _(agent-verified 2026-06-18 — excellent already (focus-trap dialog, inert bg, adaptive orientation, RTL, portrait toggle); added `touch-action:none` on the backdrop (no scroll-through); awaiting Ben's visual pass)_ · [x] **Accordion** _(stable 2026-06-18 — both gates; mobile chrome kit + coarse ≥44px header floor (`--valet-acc-hit`, 40px compact). Accordion.Item promoted too.)_ · [x] **Tabs** _(stable 2026-06-18 — both gates; chrome kit completion + coarse ≥44px tab floor (`--valet-tab-hit`, 40px compact); meta fixed (`variant`→orientation/placement, added examples). Tabs.Tab + Tabs.Panel promoted too.)_ (→ Tooltip, now locked)
